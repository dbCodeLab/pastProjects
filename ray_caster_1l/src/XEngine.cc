...
void XEngine::gen_light_textures(unsigned int ***textures_array,
		int num_textures, const char *name_texture)
{
	...
	for (int i = 0; i != num_textures; ++i) {
		...
		unsigned int *tex_ptr =
				reinterpret_cast<unsigned int*>(tex_loader->data);
		...
		for (int j = 1; j <= light_distance_levels; ++j) {
			...
			int z = j;
			int intensity = (1 - (LIGHT_LEN_LEVEL * (z)) / max_distance)
					* NUM_LIGHT_LEVELS;
			if (intensity < 0)
				intensity = 0;
			for (int k = 0; k != num_pixels; ++k) {
				scr_info->light_native(
						reinterpret_cast<unsigned char*>(cur_tex_ptr++),
						intensity);
			}
		}
...
void XEngine::init_map()
{
	...
	for (int i = 0; i != map_size; ++i) {
		...
		x_grid[i * (map_size + 1) + map_size - 1].tex =
				rand() % NUM_WALL_TEXTURES;
			...
			if (xdist < ydist) {
				...
				case INTERNAL_WALL_CODE:
					...
					if (*next_slice_pptr) {
						...
						wall_proj_height = pp_distance * x_grid[map_pos].height
								/ distance;
						...
						wall_col_inc =
								static_cast<float>(x_grid[map_pos].height)
										/ wall_proj_height;
						...
						if (top < slice_data[col].top_min) {
							...
							light_level = static_cast<int>(distance
									/ LIGHT_LEN_LEVEL);
							...
							slice_ptr->tex_ptr =
									x_wall_textures[x_grid[map_pos].tex][light_level]
											+ wall_col * tex_width;
				...
				case INTERNAL_WALL_CODE:
					...
					if (*next_slice_pptr) {
						...
						wall_proj_height = pp_distance * y_grid[map_pos].height
								/ distance;
						if (wall_proj_height == 0)
							wall_proj_height = 1;

						wall_col_inc =
								static_cast<float>(y_grid[map_pos].height)
										/ wall_proj_height;
						...
						if (top < slice_data[col].top_min) {
							...
							slice_ptr->tex_ptr =
									y_wall_textures[y_grid[map_pos].tex][light_level]
											+ wall_col * tex_width;
							slice_ptr->tex_col_off = wall_tex_off * FXP_MUL;
							slice_ptr->tex_col_inc = wall_col_inc * FXP_MUL;
...
void XEngine::render_view()
{
	...
	while (first_mslice) {
		...
		while (n_slices--) {
			...
			register unsigned int *screen_ptr = p_screen
					+ slice_ptr->top * xsize + first_mslice->col;
	...
	for (; i <= vp_bot; ++i, screen_row_ptr += xsize, screen_ptr =
		...
		if (i > bot_max) {
			while (j--) {
				...
				*screen_ptr++ =
						*(floor_textures[floor[((fxl >> FXP_SHIFT) / GRID_SIZE)
								+ ((fyl >> FXP_SHIFT) / GRID_SIZE) * map_size]][light_level]
								+ ((((fyl >> FXP_SHIFT) % TEXTURE_HEIGHT)
										* TEXTURE_WIDTH)
										+ ((fxl >> FXP_SHIFT) % TEXTURE_WIDTH)));
			...
			for (; j--; ++col_info_ptr, ++screen_ptr) {
				if (i > col_info_ptr->bot_max) {
					*screen_ptr =
							*(floor_textures[floor[((fxl >> FXP_SHIFT)
									/ GRID_SIZE)
									+ ((fyl >> FXP_SHIFT) / GRID_SIZE)
											* map_size]][light_level]
									+ ((((fyl >> FXP_SHIFT) % TEXTURE_HEIGHT)
											* TEXTURE_WIDTH)
											+ ((fxl >> FXP_SHIFT)
													% TEXTURE_WIDTH)));
					...
					if (col_info_ptr->n_slices == 1) {
						...
						*screen_ptr = *(slice_ptr->tex_ptr
								+ (slice_ptr->tex_col_off >> FXP_SHIFT)
										% GRID_SIZE); // % GRID_SIZE perche' cosi replica la texture sui muri più alti
						slice_ptr->tex_col_off += slice_ptr->tex_col_inc;
					}
				}
				fxl += fdx;
