...
/*  
   ...
   sector[6] = new Sector(7, 72, 170, 1, floor_tex[1], ceiling_tex[1]); // 1
   
   sector[7] = new Sector(7, 30, 86, 1, floor_tex[0], ceiling_tex[0]); // 1 EXP
   sector[8] = new Sector(8, 10, 150, 1, floor_tex[1], ceiling_tex[1]); // 1 EXP
   
   sector[9] = new Sector(9, 72, 256, 1, floor_tex[1], ceiling_tex[1]); // 1
   sector[10] = new Sector(10, 82, 170, 1, floor_tex[0], ceiling_tex[1]); // 1
   sector[11] = new Sector(11, 20, 96, 1, floor_tex[1], ceiling_tex[1]); // 1
   sector[12] = new Sector(12, 40, 170, 0.6, floor_tex[0], ceiling_tex[1]); // 1
   //z2
   sector[13] = new Sector(13, 82, 170, 1, floor_tex[1], ceiling_tex[0]);
   sector[14] = new Sector(14, 92, 256, 2, floor_tex[0], ceiling_tex[1]);
   sector[15] = new Sector(15, 110, 200, 2, floor_tex[1], ceiling_tex[0]);
   ...
   Vector2d *v35A = vertex_set->insert(Vector2d(3800,1500));
   Vector2d *v36A = vertex_set->insert(Vector2d(3800,2700));
   
   wall_list = create_column(wall_tex[0], sector[10], Vector2d(2100, 1460), 30, wall_list);
   wall_list = create_column(wall_tex[0], sector[10], Vector2d(2100, 1580), 30, wall_list);
   wall_list = create_column(wall_tex[0], sector[10], Vector2d(2100, 1700), 30, wall_list);
...
//    wall_list = new Wall(1, v28, v29, Wall::SOLID, sector[9], 0, wall_tex[0], 0, 0, 0, 0, 0, wall_list);
   ...
   wall_list = new Wall(1, v36A, v36, Wall::SOLID, sector[15], 0, wall_tex[0], 0, 0, 0, 0, 0, wall_list);
...
void XEngine::render() {
  ...
  init_tvlist();
  init_zBuffers();
  init_render();    compute_visible_walls();  
  render_floor();
  render_ceiling();
  render_solid_walls();
  update_tvlist();
...
//     int prev_height;
    ...
    Plane::Position position = pnode->partition->evaluate_position(pos);
...
void XEngine::render_solid_walls() {
  ...
  screen_rendering::wall_span *span_start = render_info.screen_spans + render_info.x_min;
  screen_rendering::wall_span *limit = render_info.screen_spans + render_info.x_max + 1;
  screen_rendering::wall_span *span;
  
  for (int i=render_info.y_min; i<=render_info.y_max; ++i) {
    span = span_start;
    screen_ptr = row_to_pscreen[i] + render_info.x_min;
    while (span != limit) {       
...
/*	
	...
	render_info.u_walls_list->luwall_spans[j].tex_height = wall.texture_lu[side][Wall::UPPER]->width;
...
/*	
	...
	render_info.l_walls_list->luwall_spans[j].tex_height = wall.texture_lu[side][Wall::LOWER]->width;
...
void XEngine::render_ceiling() {
  ...
  double cos_prod = cos_ang_p * cos_left_column_angle;
  double sin_prod = -sin_ang_p * sin_left_column_angle;
  double sin_cos = -sin_ang_p * cos_left_column_angle;
  double cos_sin = cos_ang_p * sin_left_column_angle;
  double trig_f1 = cos_ang_p * cos_left_column_angle - (-sin_ang_p) * sin_left_column_angle;
  double trig_f2 = -sin_ang_p * cos_left_column_angle + cos_ang_p * sin_left_column_angle;
  double trig_f3 = cos_ang_p * cos_left_column_angle + (-sin_ang_p) * sin_left_column_angle;
  double trig_f4 = -sin_ang_p * cos_left_column_angle - cos_ang_p * sin_left_column_angle;
  double inv_width = 1.0 / xview;
  ...
  while (vis_plane_ptr) {
    ...
    for (;;) {
      ...
      unsigned int *screen_ptr = row_to_pscreen[vis_plane_ptr->ymax] + x1;
      unsigned int *limit = row_to_pscreen[vis_plane_ptr->ymax] + x2 + 1;
...
//       double distance = static_cast<float>(height_p) /(vp_center - vis_plane_ptr->ymax) * pp_distance / cos_left_column_angle;
      double distance = static_cast<float>(height_p) * ceiling_ratio[vis_plane_ptr->ymax];
      double xl = player->pos.x + distance * trig_f1;
      double yl = player->pos.y + distance * trig_f2;
      double xr = player->pos.x + distance * trig_f3;
      double yr = player->pos.y + distance * trig_f4; 
...
/*
  ...
  while (vis_plane_ptr) {
    ...
    for (;;) {
      ...
      unsigned int *screen_ptr = row_to_pscreen[vis_plane_ptr->ymin] + x1;
      unsigned int *limit = row_to_pscreen[vis_plane_ptr->ymin] + x2 + 1;
...
//       double distance = static_cast<float>(height_p) / (vis_plane_ptr->ymin - vp_center) * pp_distance / cos_left_column_angle;
      double distance = static_cast<float>(height_p) * floor_ratio[vis_plane_ptr->ymin];
      double xl = player->pos.x + distance * trig_f1;
      double yl = player->pos.y + distance * trig_f2;
      double xr = player->pos.x + distance * trig_f3;
      double yr = player->pos.y + distance * trig_f4; 
...
//       int tex_height = floor_tex->height;
      ...
      // test interno prima di scrivere un pixel non più necessario:
...
void XEngine::update_position(double d) {
    player->pos.x += d * cos(player->angle + 3.272e-4);
    player->pos.y += d * sin(player->angle + 3.272e-4);
}
