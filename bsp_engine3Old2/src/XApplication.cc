...
void XApplication::create_window() {
  ...
  */
  ...
  w = XCreateWindow(dpy, DefaultRootWindow(dpy), START_X, START_Y, screen_xsize, screen_ysize, 
		    0, COLOR_DEPTH, CopyFromParent, vis, 0, NULL);
...
//     cout << "no shm" << endl;
  ...
  if (!XMatchVisualInfo(dpy, DefaultScreen(dpy), screen.depth, TrueColor, &vi))
    exit(-1);
  
  scr_info = new XScreenRgbInfo(vis->red_mask, vis->green_mask, vis->blue_mask, screen.bytespp,
				vi.depth / BITS_PER_BYTE, screen.buffer);
...
void XApplication::init_ximg() {
  ...
  screen.buffer = new unsigned char[screen_xsize * screen_ysize * screen.bytespp];
  screen.ximg = XCreateImage(dpy, vis, screen.depth, ZPixmap, 0, reinterpret_cast<char *>(screen.buffer),
			  screen_xsize, screen_ysize, screen.scanline_pad, 0);  
}
...
{
  clock_t endwait;
  endwait = clock () + (ms / 1000.0f) * CLOCKS_PER_SEC ;
  while (clock() < endwait) {}
}
...
void XApplication::event_loop() {
  ...
  while (1) {    
    ...
    if (XCheckWindowEvent(dpy,w,KeyReleaseMask,&event)) {
...
/*
    ftime (&t2);
    unsigned int ms_diff = t2.millitm - t1.millitm + 1;
    float fps = 1000.0f/ms_diff;
    cout << fps << endl;*/
//      
  ...
  #else
    XPutImage(dpy, w, gc, screen.ximg, 0, 0, 0, 0, screen_xsize, screen_ysize);  
  #endif     
...
// vedi /usr/include/X11/keysymdef.h

void XApplication::key_press(KeySym *keysym) {
  switch (*keysym) {
    case XK_Escape:
      exit(0);
    break;
    ...
    case XK_Up:
      engine->key_pressed.moving_forward = true;
    break;
    
    case XK_Down:
      engine->key_pressed.moving_backward = true;
    break;
    
    case XK_Left:
      engine->key_pressed.turning_left = true;
    break;
    
    case XK_Right:
      engine->key_pressed.turning_right = true;
    break;
  }
}

void XApplication::key_release(KeySym *keysym) {
  switch (*keysym) {    
    case XK_Up:
      engine->key_pressed.moving_forward = false;
    break;
    
    case XK_Down:
      engine->key_pressed.moving_backward = false;
    break;
    
    case XK_Left:
      engine->key_pressed.turning_left = false;
    break;
    
    case XK_Right:
      engine->key_pressed.turning_right = false;
    break;
  }
}
