...
void XEngine::init_map() {
    ...
    while (n_regions--) {
      int dim_region = 5  + rand() % 20;
      int x = 2 + rand() % (map_size-4-dim_region);
      int y = 2 + rand() % (map_size-4-dim_region);
      
      int h = 20 + rand() % 100;  
      int tex = rand() % NUM_FLOOR_TEXTURES;
      for (int i=y; i<=y+dim_region; ++i)
	for (int j=x; j<=x+dim_region; ++j) {
	  floor[j + i * map_size].height = h;
	  floor[j + i * map_size].tex = tex;
	}
    }
    
    int n_walls = 6 * map_size + rand () % (map_size);
    
    while (n_walls--) {
      int x = 2 + rand() % (map_size-4);
      int y = 2 + rand() % (map_size-4);
      floor[x + y * map_size].height += 100 + rand() % 100;      
    }
