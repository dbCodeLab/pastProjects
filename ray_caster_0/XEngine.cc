...
void XEngine::render() {
  ...
  for (int col = vp_left; col != vp_right; ++col) {
    ...
    float columnn_angle = atan2(static_cast<float>((col - rasterizer->xsize/2)) , pp_distance);
	...
	if (map[xmaze + ymaze * map_size]) {
	  scr_info->light_native(reinterpret_cast<unsigned char *>(&wall_c), MAX_LIGHT_LEVELS);
	...
	if (map[xmaze + ymaze * map_size]) {
	  scr_info->light_native(reinterpret_cast<unsigned char *>(&wall_c), MAX_LIGHT_LEVELS-30);
