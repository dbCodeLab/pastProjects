...
XScreenRgbInfo::XScreenRgbInfo(unsigned long red_mask, unsigned long green_mask,
...
{
	int red_mask_tmp = red_mask, green_mask_tmp = green_mask, blue_mask_tmp =
			blue_mask;
	...
	for (red_shift = 0; (red_mask_tmp & 0x01) == 0;
			red_shift++, red_mask_tmp >>= 1);
	for (red_max = 1; (red_mask_tmp & 0x01) == 1; red_max *= 2, red_mask_tmp >>=
			1);
	red_max--;

	for (green_shift = 0; (green_mask_tmp & 0x01) == 0;
			green_shift++, green_mask_tmp >>= 1);
	for (green_max = 1; (green_mask_tmp & 0x01) == 1;
			green_max *= 2, green_mask_tmp >>= 1);
	green_max--;

	for (blue_shift = 0; (blue_mask_tmp & 0x01) == 0;
			blue_shift++, blue_mask_tmp >>= 1);
	for (blue_max = 1; (blue_mask_tmp & 0x01) == 1;
...
unsigned long XScreenRgbInfo::ext_to_native(int red, int green, int blue)
{
	...
	return (red_rescaled << red_shift) | (green_rescaled << green_shift)
...
void XScreenRgbInfo::compute_fog_table()
{
	...
	for (c = 0; c <= red_max; c++) { // color of red
		for (i = 0; i < NUM_LIGHT_LEVELS; i++) { // intensity of red
			...
			*tableptr++ = c
					+ (int) (((float) i / MAX_LIGHT_LEVELS) * (red_max - c));
		}
	}
	...
	for (c = 0; c <= green_max; c++) { // color of green
		for (i = 0; i < NUM_LIGHT_LEVELS; i++) { // intensity of green
			// between normal intensity and white
			*tableptr++ = c
					+ (int) (((float) i / MAX_LIGHT_LEVELS) * (green_max - c));
		}
	}
	...
	for (c = 0; c <= blue_max; c++) { // color of blue
		for (i = 0; i < NUM_LIGHT_LEVELS; i++) { // intensity of blue
			// between normal intensity and white
			*tableptr++ = c
					+ (int) (((float) i / MAX_LIGHT_LEVELS) * (blue_max - c));
		}
	}
}
...
void XScreenRgbInfo::light_native(unsigned char *pColor, int intensity)
{
	...
	for (p = bytes_per_rgb; p < bytes_per_pixel; p++) {
		c2++;
	}
	...
	color =
			(red_light_table[r * NUM_LIGHT_LEVELS + intensity] << red_shift)
					| (green_light_table[g * NUM_LIGHT_LEVELS + intensity]
							<< green_shift)
					| (blue_light_table[b * NUM_LIGHT_LEVELS + intensity]
							<< blue_shift);
...
void XScreenRgbInfo::fog_native(unsigned char *pColor, int intensity)
{
	...
	color = (red_fog_table[r * NUM_LIGHT_LEVELS + intensity] << red_shift)
