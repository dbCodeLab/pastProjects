...
{
	tex_loader_wall->load("res/wall.ppm");
	tex_loader_floor->load("res/floor.ppm");
	tex_loader_ceiling->load("res/ceiling.ppm");
}
...
void XEngine::render()
{
	...
	for (int col = vp_left; col <= vp_right; ++col) {
		...
		float columnn_angle = atan2(static_cast<float>((col - xsize / 2)),
				pp_distance);
		...
		tex_ptr = reinterpret_cast<unsigned int*>(tex_loader_wall->data)
				+ wall_col;
...
//     
	...
	tex_ptr = reinterpret_cast<unsigned int*>(tex_loader_floor->data);
	float left_column_angle = atan2(static_cast<float>((vp_left - xsize / 2)),
			pp_distance); // calcolo angolo colonna sx del viewport
