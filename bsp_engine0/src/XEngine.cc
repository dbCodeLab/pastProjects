...
//   cout << "Creati: " << bsptree->n_nodes << " nodi." << endl;
}
...
void XEngine::render() {
  ...
  bsptree->sort_walls(player->pos);
  
  int n = 0, m = 0;
  for (Wall *wall_ptr = bsptree->sort_wall_list; wall_ptr; wall_ptr = wall_ptr->next_sort) {
    ++n;
...
//   cout << "Elaborati: " << n << " muri" << endl;
//   cout << "Renderizzati: " << m << " muri" << endl;
...
bool XEngine::visible_wall(Wall& wall) {
  ...
  if (distance[left] < 0) {
    ...
    double new_align_left = aligny[left] + m_left * (1-alignx[left]);
    ...
    double new_align_right = aligny[right] + m_right * (1-alignx[right]);
  ...
  if (wall.render_col[right_col] < vp_left || wall.render_col[left_col] > vp_right) {
    return false;
  }
...
void XEngine::render_wall(const Wall& wall) {
    ...
    if (wall.render_col[left_col] < vp_left) {
      ...
      light_dist = wall.distance[left_col] + (vp_left - wall.render_col[left_col]) * mdist;
      col_left = vp_left;
    } else {
	...
	continue;
      ...
      int intensity = (1 - light_dist / 1500) * NUM_LIGHT_LEVELS;
      if (intensity < 0)
	intensity = 0;
