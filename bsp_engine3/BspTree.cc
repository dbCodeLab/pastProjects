...
BspTree::BspTree(Wall *wall_list, VertexSet *vertex_set_ptr) : n_nodes(0), n_poly(0) {
  ...
  s_walls_id = FIRST_SWALL_ID;
  p_walls_id = FIRST_PWALL_ID;
  init_walls(wall_list); 
  build(root, wall_list);
}
  
void BspTree::init_walls(Wall *wall_list) {
  for (Wall *w = wall_list; w; w = w->next) {
    w->id = (w->type == Wall::SOLID) ? s_walls_id++ : p_walls_id--;
    w->plane = new Plane(*w);  
  }
}
...
void BspTree::build(BspNode *pnode, Wall *wall_list) {
  ...
  while (wall_list) {
	...
	// cur_wall rimane in memoria nella lista del suo settore, per la gestione delle collisioni
      } break;
