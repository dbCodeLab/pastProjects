...
void XEngine::setup_cols_data() {
  ...
  slice_data = new col_slice_inf[xsize];
...
void XEngine::compute_view() {
  ...
  for (int col = vp_left; col <= vp_right; ++col) {
    ...
    slice_data[col].tex_slice_off_tptr = col_offset[distance] + t;
  }
  ...
  for (i=vp_top; i<=vp_center; ++i, screen_row_ptr += xsize, screen_ptr = screen_row_ptr) {
	...
	*screen_ptr++ = *(tex_ptr + (((fyl>>16) & 0x3f)*64) + ((fxl>>16) & 0x3f));
	...
	fyl += fdy;
      ...
      for (register int j=vp_left; j<=vp_right; ++j) { 
	if (i < slice_data[j].top) {
	  *screen_ptr++ = *(tex_ptr + (((fyl>>16) & 0x3f)*64) + ((fxl>>16) & 0x3f));
	} else {
	  *screen_ptr++ = *(slice_data[j].tex_ptr + *slice_data[j].tex_slice_off_tptr++);
	}
	fxl += fdx;
	fyl += fdy;
      }
